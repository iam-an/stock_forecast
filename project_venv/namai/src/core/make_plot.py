...
def make_plot_time(train, test, y_pred_test, output, fig_name):
    # Prophet 予測の中心値 (yhat)
    y_pred = y_pred_test["yhat"].values
    fig, ax = plt.subplots(figsize=(7,3))
    # numpy配列に変換してから扱う
    ds_train = train["ds"].to_numpy()
    y_train = train["y"].to_numpy()
    ds_test = test["ds"].to_numpy()
    ...
    yhat_lower = np.concatenate([[y_pred[0]], y_pred_test["yhat_lower"].to_numpy()])
    yhat_upper = np.concatenate([[y_pred[0]], y_pred_test["yhat_upper"].to_numpy()])
